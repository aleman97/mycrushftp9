...
var taskDesigner = {
            ...
            , function(curItem, index){
                ...
                if(name && name != "undefined" && name.indexOf("_")!=0)
                {
                    if(type.toLowerCase() != "dir" && (!curItem.children || curItem.children.length==0))
                    {
                        ...
                        return $("<li class='job' _id='"+escape(id)+"' _index='"+index+"' _name='"+name+"'><div><span class='listText'>" + name + "</span></div><div class='meta "+enabled+"'><span class='type'><strong>"+type+"</strong>"+ nextRun +"</span><span class='dates'>"+ created + modified +"</span></div><span class='favorite'>"+taskDesigner.getFavorites(id)+"</span></li>");
                    }
                    else{
            ...
            if(newFolderName != null && newFolderName.length>0)
            {
                ...
                function continueAdding(){
                    ...
                    crushFTP.data.serverRequest(obj, function(data){
                        ...
                        if(status.toLowerCase().indexOf("failure")!=0)
                        {
                            crushFTP.UI.growl("Success", status, false, 5000);
                            taskDesigner.showLoading(true);
                        ...
                        {
                            crushFTP.UI.growl("Error while creating a new folder", "" + status, true);
                            taskDesigner.showLoading(true);
                        }
                    });
                }
    ...
    deleteFolder : function(currentName, cb){
        jConfirm("Are you sure you wish to delete folder \"" + currentName + "\" and all jobs in the folder", "Confirm", function(flag){
            if(flag)
            {
                function continueRenaming(){
                    taskDesigner.showLoading();
							...
							if(data)
							{
                                if($(this).is("textarea") && $.sceditor && $(this).sceditor("instance")){
                                    ...
                                    if(curVal.indexOf("<html>")<0)
                                        curVal = '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>' + curVal + '</body></html>';
			...
			else if($(this).attr("id") == "copyJobsSchedule")
			{
				...
				if(toCopy.length>0)
				{
                    ...
                    if(toCopy.hasClass('folder')){
                        crushFTP.UI.growl("Error", "Please select a job to copy", true, 3000);
                        return false;
                    }
					if(typeof _index != "undefined")
					{
                        ...
                        if(jobName){
                            ...
                            crushFTP.UI.growl("Job copied: " + jobName.name, "Click on paste button to copy the job to current location", false, 3000);
                        }
					}
				}
			}
            else if($(this).attr("id") == "pasteJobsSchedule")
            {
                ...
                taskDesigner.getJobDetailsByName(jobName, function(controlData, xml){
                    if(controlData)
                    {
                        ...
                        jPrompt("Job Name", "Copy Of " + taskDesigner.copiedJobName.name, "Input", function(scheduleName){
                            if(scheduleName != null && scheduleName.length>0)
                            {
                                scheduleName = $.trim(scheduleName);
                                scheduleName = folderName + scheduleName;
    				...
    				if(dataItem)
    				{
    					...
    					if(taskDesigner.formDialog)
    					{
                            ...
                            else if(controlData.type == "Email")
                            {
                                ...
                                var plainTextBody = controlData.emailBodyPlainText && controlData.emailBodyPlainText == "true";
	...
	bindFormEvents : function(form, controlData, callback){
        ...
        if(taskDesigner.kioskData && window.parent)
        {
            v10Beta = window.parent.$(window.parent.document).data("GUIXMLPrefs") && window.parent.$(window.parent.document).data("GUIXMLPrefs").v10_beta === "true";
        }
        if(v10Beta){
            form.find(".v10beta").removeClass('v10beta');
            $("#pgpPrivateKeyUploadPath, #pgpPublicKeyUploadPath", form).each(function(){
            ...
            var keyInput = $("#ssh_private_key", form).each(function(){
        ...
        $("#mail_host, #smtp_server", form).bind("textchange", function() {
            if ($(this).val().indexOf('gmail.com')>=0) {
                $(".googleToken", form).show();
            } else {
                $(".googleToken", form).hide();
            }
        });
        ...
        $("a.serverFilePickTestButton", form).each(function(){
            $(this).unbind("click").click(function(evt){
                ...
                if(!pathToVerify){
                    ...
                    var suggestions = $(".suggested:visible", "#taskFormDialog").find("#suggestedSettingsUsed").find("li");
                    if(suggestions && suggestions.length>0){
                        suggestions.each(function(){
                            if($(this).attr("key")){
