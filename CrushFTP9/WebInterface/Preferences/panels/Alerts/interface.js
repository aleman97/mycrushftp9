...
panelAlerts.defaultVals = {
	...
	loginFrequency : {
		body : "User {user_name} is logging in too frequently.  They logged in {alert_msg} times in the past {alert_login_interval} seconds."
	},
	loginFrequencyBanned : {
		body : "User {user_name} has been banned for too frequent of logins.  They had {count} logins (max of {attempts}) in {interval} seconds.  They are banned until {expire}."
	},
	big_dir : {
		body : "{alert_msg}"
	},
...
panelAlerts.bindEvents = function()
{
	...
	_panel.find("select#type").change(function(){
		...
		if(type == "Login Frequency")
		{
			$("#body", _panel).val(panelAlerts.defaultVals.loginFrequency.body).change();
		}
		if(type == "Login Frequency Banned")
		{
			$("#body", _panel).val(panelAlerts.defaultVals.loginFrequencyBanned.body).change();
		}
		...
		if(type == "Slow Login")
		{
			$("#body", _panel).val(panelAlerts.defaultVals.slow_login.body).change();
			typeText = "Slow Login";
		}
		if(type == "Job Changes")
		{
			$("#body", _panel).val(panelAlerts.defaultVals.job_changes.body).change();
			typeText = "Job Changes";
		}
