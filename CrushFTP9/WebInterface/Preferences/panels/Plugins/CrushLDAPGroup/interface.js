...
pluginCrushLDAPGroup.bindData = function(index, pluginID)
{
	...
	if(pluginPrefs)
	{
		...
		$("#server_items_list", _plugin).find("li").removeClass('ui-selected ui-widget-header ui-state-disabled');
		if(server_item)
		{
		...
		$("#user_connection_groups", _plugin).find("li").removeClass('ui-selected ui-widget-header ui-state-disabled');
...
pluginCrushLDAPGroup.bindEvents = function()
{
	...
	server_items_list.find("li").unbind("click").bind("click", function(e) {
        ...
        if(server_items_list.find("#server_item_all").hasClass('ui-selected'))
        {
            server_items_list.find("li[name!='server_item_all']").each(function(){
                $(this).closest("li").addClass('ui-state-disabled').removeClass('ui-selected ui-widget-header');
            });
        }
        else{
        	server_items_list.find(".ui-state-disabled").removeClass('ui-state-disabled');
        }
        lastClickedSLI = $(this);
        itemsChanged(true);
		panelPlugins.itemsChanged(true);
	...
	user_connection_groups.find("li").unbind("click").bind("click", function(e) {
        ...
        if(user_connection_groups.find("#server_group_all").hasClass('ui-selected'))
        {
        	...
        	user_connection_groups.find(".ui-state-disabled").removeClass('ui-state-disabled');
        }
        lastClickedCG = $(this);
        itemsChanged(true);
		panelPlugins.itemsChanged(true);
	...
	if(!this.serverListShown)
	{
		...
		for(var i=0;i<serverList.length;i++)
		{
			...
			if(curItem)
			{
				...
				var newControl = $('<li class="ui-widget-content" server_name="'+server+'" name="server_item_'+server+'" id="server_item_'+server+'">' + server + '</li>');
		...
		for(var i=0;i<groupList.length;i++)
		{
			...
			if(curItem)
			{
				...
				var newControl = $('<li class="ui-widget-content" server_group="'+serverGroup+'" name="server_group_'+serverGroup+'" id="server_group_'+serverGroup+'">' + serverGroup + '</li>');
