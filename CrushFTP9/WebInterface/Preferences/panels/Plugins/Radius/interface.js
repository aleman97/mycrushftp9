...
var pluginRadius = {};
pluginRadius.localization = {};
/****************************/

// Plugin details
var pluginName = "Radius";
...
localizations.panels["Plugins"][pluginName] = $.extend(pluginRadius.localization, localizations.panels["Plugins"][pluginName]);

// Interface methods
pluginRadius.init = function(pluginID, returnXML){
	pluginRadius.returnXML = returnXML;
	applyLocalizations(pluginName, localizations.panels["Plugins"]);
	pluginRadius.bindData(0, pluginID);
...
pluginRadius.bindData = function(index, pluginID)
{
	index = index || 0;
	var pluginPrefs = [];
	pluginRadius.showServerList();
	if(pluginID)
	{
	...
	if(pluginPrefs)
	{
		...
		pluginRadius.bindPluginDetails(curPlugin);
	}
	pluginRadius.loadedSubItem = index;
	pluginRadius.bindEvents(index);
...
pluginRadius.bindEvents = function()
{
	...
	_plugin.find("input, select, textarea").bind("change", function(){
		...
		panelPlugins.itemsChanged(true, pluginRadius.returnXML, pluginName);
	});
	...
	homeDirSubFolders.selectableAdvanced({
		select: function(event, ui) {
			...
			pluginRadius.showSubFolderPrivs();
			return false;
		},
	...
	var radiusRolesList = $("#radius_roles", _plugin);
...
pluginRadius.editRoles = function(){
	var radiusRolesList = $("#radius_roles", _plugin);
	var roles = "";
...
pluginRadius.showSubFolderPrivs = function(){
	var homeDirSubFolders = $("#homeDirSubFolders", _panel);
	var item = homeDirSubFolders.find("li.ui-selected");
...
pluginRadius.bindPluginDetails = function(controlData)
{
	...
	var radiusRolesList = $("#radius_roles", _plugin).empty();
...
pluginRadius.showServerList = function()
{
	if(!this.serverListShown)
	{
		var service = common;
		if(pluginRadius.returnXML)
		{
			service = crushFTP;
		}
...
pluginRadius.saveContent = function(saveByIndex, cloneName, removeByIndex, callback, noGrowl)
{
	removeByIndex = removeByIndex || 0;
	if(pluginPlaceHolder.data("hasChanged") || removeByIndex>0 || (saveByIndex>0 && cloneName) || pluginRadius.returnXML)
	{
		if(!pluginRadius.returnXML)
		...
		if(removeByIndex == 0)
		{
			...
			var radiusRoles = $("#radius_roles", container).find("li");
