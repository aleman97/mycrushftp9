...
var optionsByType = {
    FILE: [], // - none
    SMB: ['timeout', 'write_timeout', 'read_timeout'], // - connection/read/write timeout
    SMB3: ['timeout', 'write_timeout', 'read_timeout', 'dfs_enabled', 'smb3_required_signing', 'use_dmz'], // - connection/read/write timeout, DFS enabled, required signing, route through DMZ
    ...
    S3: ['timeout', 'write_timeout', 'read_timeout','s3_accelerate','s3_stat_head_calls','s3_sha256','s3_partial','s3_bucket_in_path','no_bucket_check','server_side_encrypt','server_side_encrypt_kms','multithreaded_s3'], // - connection/read/write timeout, bucket, accelerate, fast s3, sha256, bucket in path, show partial uploads, don't check bucket at login, server side encrypt, kms key id, multithread
    S3Crush: ['timeout', 'write_timeout', 'read_timeout','s3_accelerate','s3_stat_head_calls','s3_sha256','s3_partial','s3_bucket_in_path','no_bucket_check','server_side_encrypt','server_side_encrypt_kms','multithreaded_s3','random_id','segmented'], // - connection/read/write timeout, bucket, accelerate, fast s3, sha256, bucket in path, show partial uploads, don't check bucket at login, server side encrypt, kms key id, multithread, use random id, store files in segments
    ...
    AZURE: ['timeout', 'write_timeout', 'read_timeout','shareName','upload_blob_type','sas_token','data_lake_storagegen2'], //- connection/read/write timeout, share name, upload blob type, shared access signature token, data lake storage gen2.
    ...
    ONEDRIVE: ['timeout', 'write_timeout', 'read_timeout','onedrive_my_shares','onedrive_share_name', 'onedriveTenant'],
    SHAREPOINT: ['timeout', 'write_timeout', 'read_timeout','onedrive_my_shares','sharepoint_site_id','sharepoint_site_path','sharepoint_site_drive_name', 'onedriveTenant'],
};
...
function getFullURLWithParams(value, params, pass){
    ...
    for(var item in params){
        paramsByType.push(fixChars(item) + valueSeparator + fixChars(params[item] || ''));
    }
    ...
    $.crushFtpLocalFileBrowserPopup = function(el, options) {
            ...
            if(base.options.isFTPBrowse)
            {
                ...
                if($("#"+ dialogID).length==0)
                {
                    ...
                    ftpDialogHTML.load("/WebInterface/Resources/templates/remoteVFSItemForm.html", function(){
                        ...
                        var v10Beta = prefs && prefs.v10_beta === "true";
                        if(!v10Beta && prefs)
                        {
                            if(prefs.v10_beta && prefs.v10_beta[0])
                                {
                                    v10Beta = prefs.v10_beta[0].text === "true";
                                }
                        }
                        if(v10Beta){
                            browsePopupFTPDialog.find(".v10beta").removeClass('v10beta');
                            ...
                            browsePopupFTPDialog.find(".v10beta").remove();
                        }
        ...
        base.bindFTPFormEvents = function(fieldPropertiesDialog){
            ...
            function showHideItemPropertiesSettings()
            {
                ...
                if(remoteVFSItem_option_itemType == "smb" || remoteVFSItem_option_itemType == "smb3" || remoteVFSItem_option_itemType == "rfile" || remoteVFSItem_option_itemType == "glacier" || remoteVFSItem_option_itemType == "backblaze")
                {
                    ftpCredentials.show();
                    HAOptions.hide();
                    noFileOption.hide();
            ...
            function buildPropertiesURL()
            {
                ...
                function clearURL(_url, except){
                    var toReplace = ["s3.amazonaws.com/","google.com/","sf-api.com/","glacier.us-east-1.amazonaws.com/","file.core.windows.net/", "storage.googleapis.com/", "api.dropboxapi.com/", "api.backblaze.com/", "graph.microsoft.com/"];
                ...
                else
                {
                    ...
                    if(!azureSelected && !smbSelected && !smb3Selected){
            ...
            function bindFTPFormData()
            {
                ...
                if(matchedElem)
                {
                        ...
                        if(url.indexOf("://")>=0)
                        {
                            ...
                            if(url.indexOf("@")>=0)
                            {
                                ...
                                if(url && url.indexOf(":")>=0)
                                {
                                    ...
                                    if(cred.length>0)
                                    {
                                        $("#remoteVFSItem_option_user_name,#remoteVFSItem_option_secretKeyID,#remoteVFSItem_option_gdrive_secretKeyID,#remoteVFSItem_option_client_id, #remoteVFSItem_option_dropbox_client_id, #remoteVFSItem_option_onedrive_secretKeyID", fieldPropertiesDialog).val(cred[0]);
                                    }
                                    if(cred.length>1)
                                    {
                                        $("#remoteVFSItem_option_password,#remoteVFSItem_option_secretKey,#remoteVFSItem_option_gdrive_secretKey,#remoteVFSItem_option_client_secret, #remoteVFSItem_option_dropbox_client_secret, #remoteVFSItem_option_onedrive_secretKey", fieldPropertiesDialog).val(cred[1]);
                                    }
                                }
                            }
                        }
                    ...
                    if(lowCaseURL && lowCaseURL.indexOf("onedrive") == 0 || lowCaseURL && lowCaseURL.indexOf("sharepoint") == 0)
                    {
            ...
            fieldPropertiesDialog.dialog({
                ...
                buttons: {
                    ...
                    "Connect": function() {
                        ...
                        function continueProcess()
                        {
                            ...
                            var glacierSelected = $("#remoteVFSItem_option_itemType", fieldPropertiesDialog).val() == "glacier";
                            var gstorageSelected = $("#remoteVFSItem_option_itemType", fieldPropertiesDialog).val() == "gstorage";
                            ...
                            var GdriveSelected = $("#remoteVFSItem_option_itemType", fieldPropertiesDialog).val() == "gdrive" || $("#remoteVFSItem_option_itemType", fieldPropertiesDialog).val() == "gstorage";
                            if(GdriveSelected)
                            {
                                fieldPropertiesDialog.find(".privateOptions").find("input").addClass("excludeXML");
                            ...
                            var OnedriveSelected = $("#remoteVFSItem_option_itemType", fieldPropertiesDialog).val() == "onedrive"
                            if(OnedriveSelected)
                            {
                                fieldPropertiesDialog.find(".privateOptions").find("input").addClass("excludeXML");
                            ...
                            var SharepointSelected = $("#remoteVFSItem_option_itemType", fieldPropertiesDialog).val() == "sharepoint";
                            ...
                            var dropboxSelected = $("#remoteVFSItem_option_itemType", fieldPropertiesDialog).val() == "dropbox";
                            if(dropboxSelected)
                            {
                ...
                open: function(){
                    ...
                    fieldPropertiesDialog.find(".refreshMSToken").unbind().click(function(){
                        ...
                        jPrompt("Client ID: ", _name, "Please enter OneDrive Oauth", function(value, other, pass, tenant){
                            if(value)
                            {
                                crushFTP.data.encryptPass(pass, function(data){
                                    if(data)
                                    {
                                        ...
                                        base.refreshMSToken(fieldPropertiesDialog, value, generatedPass, tenant[1] || "common");
                                    }
                                });
                            }
                        }, false, false, {
        ...
        base.refreshMSToken = function(dialog, client_id, pass, tenant){
            var baseURL = window.location.protocol + "//" + window.location.host + "/";
            var curURL = encodeURIComponent(baseURL) + "custom_callback_onedrive/";
