...
panelRestrictions.init = function(){
	...
	if(v10Beta){
        $("#filePublicEncryptionKey,#fileDecryptionKey").each(function(){
...
panelRestrictions.bindData = function(userInfo, jsonDeep, panel)
{
	...
	if(userInfo.user)
	{
		...
		if(typeof userInfo.user.min_pass_age == "undefined")
			userInfo.user.min_pass_age = "0";
		...
		if(!panel)
   		...
   		panelRestrictions._panel.find(".maskPasswordOnURL").trigger("applymask");
...
panelRestrictions.bindEvents = function()
{
	...
	panelRestrictions._panel.find(".maskPasswordOnURL").each(function(){
	...
	$("a.serverFilePickWithParamsButton", panelRestrictions._panel).each(function(){
        $(this).unbind("click").click(function(){
            var curElem = $(this);
            var rel = curElem.attr("rel");
            var pickingFor = curElem.parent().parent().parent().find("label[for='"+rel+"']").text() || "";
            pickingFor = $.trim($.trim(pickingFor).replace(":", ""));
            var refElem = $("#" + curElem.attr("rel"), panelRestrictions._panel);
            ...
            curElem.crushFtpLocalFileBrowserPopup({
                ...
                callback : function(selectedPath){
                    $("#" + curElem.attr("rel"), panelRestrictions._panel).val(selectedPath).removeData("urlParams").trigger('applymask');
                    panelRestrictions._panel.trigger("changed", [refElem]);
                }
            });
            return false;
        });
	...
	$("#restrictions_pgpEncryptionGenerateButton").unbind().click(function () {
		...
		var obj = {
			...
			encryption_cypher: crushFTP.methods.htmlEncode($("#encryption_cypher_gen").val().join(",")),
			random: Math.random()
		};
