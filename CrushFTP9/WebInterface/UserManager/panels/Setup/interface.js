...
panelSetup.init = function(){
	...
	if(v10Beta){
		panelSetup._panel.find(".v10beta").removeClass('v10beta');
    	...
    	panelSetup._panel.find(".v10beta").remove();
    }
...
panelSetup.bindEvents = function(callBack)
{
	...
	function continueVFSAddProcess(callBack)
	{
		...
		function addItem(){
			...
			{
				fieldPropertiesDialog.find("input[name='random_id'], input[name='segmented'], input[name='multithreaded_s3'], input[name='s3_stat_head_calls']").removeClass("excludeXML");
			}
	...
	function refreshMSToken(dialog, client_id, pass, tenant){
		var baseURL = window.location.protocol + "//" + window.location.host + "/";
		            ...
		            if(labelName.length>0)
		            {
		                ...
		                try{
		                    ...
		                    if(url && pass)
		                    {
		                        ...
		                        url.password(mask);
		                        text = unescape(url.toString().replace(/\:\/\/\//g, '://'));
		                    }
		                }catch(ex){}
		            ...
		            if(value)
		            {
		            	crushFTP.data.encryptPass(pass, function(data){
							if(data)
							{
		            			...
		            			refreshMSToken(fieldPropertiesDialog, value, generatedPass, tenant[1] || "common");
		            		}
		            	});
		            }
		        }, false, false, {
	...
	fieldPropertiesDialog.find("#itemProperty_option_user_name, #itemProperty_option_password, #itemProperty_option_secretKeyID, #itemProperty_option_secretKey, #itemProperty_option_gdrive_secretKeyID, #itemProperty_option_gdrive_secretKey,#itemProperty_option_s3_bucket,#itemProperty_option_vault_name,#itemProperty_option_s3_path, #itemProperty_option_client_secret, #itemProperty_option_client_id, #itemProperty_option_dropbox_client_secret, #itemProperty_option_dropbox_client_id,#itemProperty_option_gstorage_bucket,#itemProperty_option_onedrive_secretKeyID,#itemProperty_option_onedrive_secretKey,#itemProperty_option_onedrive_tenant").bind("textchange", function(){
		...
		panelSetup.buildPropertiesURL();
		if($(this).is("#itemProperty_option_user_name") || $(this).is("#itemProperty_option_password") || $(this).is("#itemProperty_option_secretKey") || $(this).is("#itemProperty_option_gdrive_secretKeyID") || $(this).is("#itemProperty_option_gdrive_secretKey") || $(this).is("#itemProperty_option_client_secret") || $(this).is("#itemProperty_option_client_id") || $(this).is("#itemProperty_option_onedrive_secretKeyID") || $(this).is("#itemProperty_option_onedrive_secretKey") || $(this).is("#itemProperty_option_onedrive_tenant"))
		{
			fieldPropertiesDialog.find(".maskPasswordOnURL").trigger("applymask");
		}
	}).bind("blur", function(){
            ...
            if(value.toLowerCase().indexOf("file://")!=0 && url)
            {
                ...
                if(pass != existingPass)
                {
                    ...
                    if(!value.toLowerCase().startsWith("file:/"))
                    {
                        elem.data("password", pass);
                        url.password(mask);
                        var _val = url.toString().replace(/\:\/\/\//g, '://');
		...
		if(selectedDir && selectedDir.length>0)
		{
			...
			$(selectedDir).each(function(){
				...
				if(typeof curItem.attr !="undefined")
				{
					...
					{
						jConfirm("This will make theitem invisible, Are you sure you want to do that?", "Confirm", function(value){
	...
	$("#quickSetVFSExpirationTime").click(function(){
		...
		jPrompt("Enter thenumber of days from now until this item expires : ", defaultDays, "Input", function(value){
...
panelSetup.bindPropertiesWindowDataForSelectedItem = function(selected)
{
	...
	if(matchedElem)
	{
        ...
        {
            matchedElem.timeout = "600000|||||";
        }
        if(typeof matchedElem.write_timeout == "undefined")
        {
            matchedElem.write_timeout = "600000|||||";
        }
        if(typeof matchedElem.read_timeout == "undefined")
        {
            matchedElem.read_timeout = "600000|||||";
        }
		...
		if(lowCaseURL.indexOf("onedrive") == 0 || lowCaseURL.indexOf("sharepoint") == 0){
			var _url = URI(matchedElem.url);
			if(matchedElem)
            {
...
panelSetup.showHideItemPropertiesSettings = function(controlData)
{
	...
	if(item_option_itemType == "smb" || item_option_itemType == "smb3" || item_option_itemType == "rfile" || item_option_itemType == "glacier" || item_option_itemType == "backblaze")
	{
		ftpCredentials.show();
		HAOptions.hide();
		noFileOption.hide();
	...
	if(item_option_itemType == "sftp" || item_option_itemType == "smb3"){
...
panelSetup.buildPropertiesURLReverse = function(){
	...
	try{
		...
		{
			var _path = _url.path().split("/").removeByVal("").shift();
...
panelSetup.buildPropertiesURL = function(params)
{
	...
	function clearURL(_url, except){
		var toReplace = ["s3.amazonaws.com/","google.com/","sf-api.com/","glacier.us-east-1.amazonaws.com/","file.core.windows.net/", "api.dropboxapi.com/", "storage.googleapis.com/", "graph.microsoft.com/"];
	...
	function addUserPassToURLGdrive(url, protocol, addUP, exceptURL)
	{
		...
		if(protocol.indexOf("gstorage")==0){
			...
			var path = url ? url + '' : '';
			if(path){
				var itms = path.split("/").removeByVal("");
				itms.shift();
				path = itms.join("/");
			}
			url = "";
			bucket = $("#itemProperty_option_gstorage_bucket", fieldPropertiesDialog).val();
	...
	{
		itemProperty_option_url.val(addUserPassToURL(staticURL, "b2://", true));
	}
	        ...
	        create: function(event, ui) {
	            ...
	            "OK" : function(){
	            	...
	            	try{
						...
						url.username(user);
						url.password(pass);
						item.url = decodeURI(url.toString().replace(/\:\/\/\//g, '://'));
						afterUserPass();
					}catch(ex){}
