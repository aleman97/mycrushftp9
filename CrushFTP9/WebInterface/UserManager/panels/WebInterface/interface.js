    ...
    templatePopupDialog.dialog({
        ...
        open: function(){
            ...
            templatePopupDialog.find("#addTemplateButton").unbind().click(function(){
                ...
                var elem = $('<li class="ui-widget-content" _name="'+encodeURIComponent(cur.name)+'"><span class="handle ui-icon ui-icon-grip-dotted-vertical"></span> <span class="name">'+cur.name+'</span><span style="display: inline-block; margin: -1px 3px 0px -8px; float: right;" class="remove pointer ui-icon ui-icon-trash"></span><span style="display: inline-block; margin: 0px 5px 0px -8px; float: right;" class="flag pointer ui-icon ui-icon-flag"></span></li>');
...
panelWebInterface.bindData = function(userInfo, jsonDeep, panel) {
    ...
    if (userInfo.user) {
        ...
        userManager.UI.multiOptionControlDataBind(userInfo.user, "web_customizations", $("#webCustomizationsList", dataPanel).empty(), function(curItem) {
            ...
            var newElem = $("<li class='ui-widget-content' defaultVal='" + itemInDefaultList.attr("defaultVal") + "' _type='" + itemInDefaultList.attr("_type") + "' htmlArea='" + itemInDefaultList.attr("htmlArea") + "' headerText='" + keyText + "' key='" + key + "'>" + keyText + " : <span class='value'>" + (value.truncate(200)) + "</span></li>");
...
panelWebInterface.bindEvents = function() {
        ...
        if (selected.length > 0) {
            ...
            if(promptVal == "EMAILBODY"){
                ...
                window.afterTemplateSetup = function(){
                    ...
                    newElem.find("span.value").text(value.truncate(200));
            ...
            if (selected.attr("_type") == "languageSelection") {
                options = ["en|English","bg|Bulgarian","cn|Chinese","cs|Czech","da|Danish","nl|Dutch","fr|French","de|German","hu|Hungarian","it|Italian","ko|Korean","pl|Polish","ro|Romanian","ru|Russian","sk|Slovak","es|Spanish","se|Swedish"];
                ...
                jPrompt(promptVal + " : ", unescape(selected.attr("defaultVal")), "Input", function(value) {
                    if (value != null) {
                        ...
                        if (isHTMLArea) {
                            ...
                            webCustomizationsList.find("li:last").find("span.value").text(value.truncate(200));
                        } else
                            webCustomizationsList.find("li:last").find("span.value").text(value.truncate(200));
                        selected.hide();
                        panelWebInterface._panel.trigger("changed", [webCustomizationsList]);
        ...
        if (selected.length > 0) {
            ...
            if(selected.attr("key") == "EMAILBODY"){
                ...
                window.afterTemplateSetup = function(){
                    ...
                    selected.data("selectedVal", (value));
                    if (isHTMLArea) {
                        selected.find("span.value").text(value.truncate(200));
                    } else
                        selected.find("span.value").text(value.truncate(200));
                    panelWebInterface._panel.trigger("changed", [webCustomizationsList]);
                    delete window.currentTemplates;
                ...
                OK: function() {
                    ...
                    function enterButtonName() {
                        jPrompt("Enter a name for this button : ", buttonName, "Button Name", function(value) {
                            if (value != null) {
                                buttonName = crushFTP.methods.htmlEncode(value);
    ...
    $("a#editMenuItem", panelWebInterface._panel).click(function() {
        ...
        if (selected && selected.length > 0) {
            ...
            var buttonName = crushFTP.methods.decode($.trim(key[1]));
                ...
                if (value != null) {
                    ...
                    else
                    {
                        if(command.indexOf("setMetaInfo")>0)
                            opnMetaPopup();
                        else if(command.indexOf("customShare")>0)
                            opnSharePopup(function(value){
                                var listClass = isSubItem ? "groupSubItem" : "";
                                crushFTP.UI.replaceItem(selected, $("<li class='ui-widget-content " + listClass + "' key='" + privs + ":" + buttonName + "'><div class='" + listClass + "'>" + privs + ":" + buttonName + "</div></li>"), {
                                    key: privs + ":" + buttonName,
