...
panelGraphs.ram_pending_bytes = $("#ram_pending_bytes", panelGraphs._panel);
panelGraphs.ram_pending_bytes_s3_download = $("#ram_pending_bytes_s3_download", panelGraphs._panel);
panelGraphs.ram_pending_bytes_s3_upload = $("#ram_pending_bytes_s3_upload", panelGraphs._panel);
...
panelGraphs.init = function() {
    ...
    window.panelDashboard && panelDashboard._panel.find("fieldset#speedInformation").find(".downSpeed").bind("mouseenter", function(ev) {
        var left = ev.pageX;
        if (left > $(window).width() - 370) {
            left = $(window).width() - 370;
        }
    ...
    window.panelDashboard && panelDashboard._panel.find("fieldset#speedInformation").find(".upSpeed").bind("mouseenter", function(ev) {
        var left = ev.pageX;
        if (left > $(window).width() - 370) {
            left = $(window).width() - 370;
        }
...
panelGraphs.refreshDataFromServer = function(isInit) {
    if(window.panelDashboard && !window.panelDashboard.isLive) return;
    ...
    var params = "current_download_speed-current_upload_speed-logged_in_users-ram_free-ram_max-ram_pending_bytes-ram_pending_bytes_s3_upload-ram_pending_bytes_s3_download-server_cpu-os_cpu-connected_unique_ips";
    crushFTP.data.serverRequest({
...
panelGraphs.bindData = {
    graphs: function(data) {
        ...
        panelGraphs.RamPendingBytes = panelGraphs.RamPendingBytes.concat(data.ram_pending_bytes);
        if (panelGraphs.RamPendingBytes.length > panelGraphs.maxRecordsToShow) {
        ...
        panelGraphs.RamS3PendingDownload = panelGraphs.RamS3PendingDownload.concat(data.ram_pending_bytes_s3_download);
        if (panelGraphs.RamS3PendingDownload.length > panelGraphs.maxRecordsToShow) {
        ...
        panelGraphs.RamS3PendingUpload = panelGraphs.RamS3PendingUpload.concat(data.ram_pending_bytes_s3_upload);
        if (panelGraphs.RamS3PendingUpload.length > panelGraphs.maxRecordsToShow) {
