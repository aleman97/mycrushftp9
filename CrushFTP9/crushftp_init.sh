#!/usr/bin/env bash
#
# Control script for CrushFTP v2.1
#
# chkconfig: - 86 14
...
get_pid_alt()
{
 ...
 CRUSH_PARENT="`$PS -a | $GREP java | $GREP $CRUSH_DIR | awk '{print $2}'`"	
}
...
#system info detect moved here
	if [ ${OS} = "BSD" ];then
		MEM="No system memory info available."	
...
case "$1" in
	...
	install)
		...
		if [ "$INSTSCRIPT" = "0" ];then
                        ...
                        echo "3 - legacy system V / upstart / BSD "
